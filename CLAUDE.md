...
- **OCR은 Tesseract.js를 Next.js 서버 프로세스 안에서 직접 실행** — 별도 호스팅 불필요, 외부 API 키/쿼터 의존 없음 (원래 PaddleOCR을 Render에 배치할 계획이었으나 무료 티어 RAM 512MB 대비 실행 메모리가 실측 ~600MB로 초과해 계속 크래시, Tesseract.js로 전환)
