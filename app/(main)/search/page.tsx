...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  return (
    <div className={`flex min-h-full flex-1 flex-col bg-bg ${matches.length > 0 ? "pb-28" : "pb-8"}`}>
      <div className="sticky top-0 z-10 bg-bg px-6 pt-6 pb-2">
        ...
        {matches.length > 0 && (
          // 필터 아이콘을 나중에 여기 오른쪽에 같이 넣을 자리 — justify-between으로 미리 비워둠
          <div className="flex items-center justify-between pt-2">
            <p className="font-body text-xs text-ink-soft">일치하는 사료 {matches.length}건을 찾았어요</p>
          </div>
        )}
      </div>

      <div className="flex flex-1 flex-col gap-6 px-6 pt-3">
        {matches.length > 0 ? (
          <>
...
function RegisterInsteadLink({ keyword }: { keyword: string }) {
  return (
    <p className="fixed inset-x-0 bottom-0 bg-bg px-6 py-4 text-center font-body text-xs text-ink-soft">
      찾는 제품이 아닌가요?{" "}
